...
function primerasCondicionales(arg){
    ...
    arg.length === 0 ? res = 'Este array no contiene elementos' : false 
    arg.length < 3 ? res = 'Este array es menor a 3' : false 
    arg.length === 3 ? res = 'Este array es igual a 3' : false 
    arg.length > 3 ? res = 'Este array es mayor a 3' : false 
}
...
function valorMaximo(arr){
    ...
    //ejemplo -> [1 , 5, 4, 10, 99 ,2, 42, 3] - deberia devolver 99  // Metodo arr Math.max 
    // Tu codigo:
    return Math.max(...arr)
}
...
function valorMinimo(arr){
    ...
    //ejemplo -> [ 5, 4, 10, 99 ,2, 42, 3] - deberia devolver 2  // Math.min
    // Tu codigo:
...
function eliminarDuplicado(arr){
    ...
    // ejemplo: [1,1,2,4,5,6,6,7,1,8,9] -> [1,2,4,5,6,7,8,9] // Metodo new Set() 
    // Tu codigo:
}
